...
if (window.location.href.indexOf('localhost') > -1) {
    URL_ROOT = 'https://localhost:44359/api/values/';
}
else {
    URL_ROOT = 'https://paintchaos.azurewebsites.net/api/Values/';
}


GRID_SIZE = 300;

//loop every 2 seconds or so and send update request

WEB_SOCKET_CONNECTION = new signalR.HubConnectionBuilder().withUrl("/messagehub").build();

WEB_SOCKET_CONNECTION.on("PAINT", handleWebSocketMessage);

WEB_SOCKET_CONNECTION.start();

function handleWebSocketMessage(imgData) {
    console.log('houston we have a message: ', imgData);

    var c = document.getElementById("canvas");
    var ctx = c.getContext("2d");

    var data = ctx.getImageData(0, 0, GRID_SIZE, GRID_SIZE);

    for (var i = 0; i < imgData.data.length; i++) {
        data.data[imgData.data[i][0]] = imgData.data[i][0];
    }
...
(function () {
     ...
     */
    let mouseDownFunction = function (mouseEvent) {
        ...
        switch (drawer.selectedShape) {
            ...
            case drawer.availableShapes.LINE_LIST:
                ...
                console.log('line list selected');
                break;
    ...
    drawer.canvas.addEventListener('pointerdown',
        /**
         * Starts drawing the chosen shape.
         *
         * @param mouseEvent The event that trigger this callback
    ...
    var mouseMoveFunction = function (mouseEvent) {
    ...
    drawer.canvas.addEventListener('pointermove',
        /**
         * If any shape other than text is being drawn, we resize it.
         *
         * @param mouseEvent The event that trigger this callback
    ...
    var mouseUpFunction = function (mouseEvent) {
        if (drawer.selectedElement && drawer.selectedShape !== drawer.availableShapes.DrawnText) {
            drawer.shapes.push(drawer.selectedElement);
    ...
    const interval = setInterval(function () {
        ...
        for (var i = 0; i < GRID_SIZE * GRID_SIZE * 4; i++) {
            if (data['data'][i]) {
                compressedData.data.push([i, data['data'][i]]);
            }
        }
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState === 4) {
                console.log("result from post", this.responseText);
            }
        };

        xhttp.send(JSON.stringify(compressedData));
    }, 2000);
